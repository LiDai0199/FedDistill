...
if __name__ == "__main__":
    ...
    parser.add_argument("--beta", type=float, default=1.0, help="Average moving parameter for pFedMe, or Second learning rate of Per-FedAvg")  # 控制着从其他模型（例如，全局模型或教师模型）接收的新参数与当前模型（学生模型或本地模型）现有参数之间的权衡。
    ...
    parser.add_argument("--num_glob_iters", type=int, default=200)  # server的全局处理行为的次数
    parser.add_argument("--local_epochs", type=int, default=20)  # user的训练周期次数
    parser.add_argument("--num_users", type=int, default=20, help="Number of Users per round")
    parser.add_argument("--K", type=int, default=1, help="Computation steps")  # 控制了在每次训练迭代中数据被分批处理的次数
