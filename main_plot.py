...
metrics_map = {
    'loss': 'Loss Per Round',
    'communication_overhead': 'Communication Overhead (bytes)',
    'training_time': 'Average Training Time for Clients (seconds)'
}

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--plot_type", type=str, default="loss", help='loss or communication overhead or training time')
    ...
    ylabel = metrics_map.get(args.plot_type, None)
    if ylabel:
        plot_metric_comparison(args, algorithms, ylabel)
    else:
        print("Invalid plot type. Please choose from [loss, communication_overhead, communication_time]")